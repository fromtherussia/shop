...
class AdminEditWholesaleDiscountsCommand extends Command {
	public function __construct () {
		parent::__construct(EDITOR);
	}
	
	private function RenderTemplate($template) {
		$template->SetParam('discounts', Discounts::GetWholesaleDiscounts());
		wrapAdminTemplate('Управление скидками', 3, $template, true)->Render();
	}
	
	public function Execute() {
		$template = new Template('AdminEditWholesaleDiscounts');
		...
		if (isExists('discount-precent', $_POST) &&
			...
			Discounts::SetWholesaleDiscounts($discounts);
			$template->SetMessage('Политика скидок успешно изменена.');
		}
		
		$template->SetParam('saveHref', Command::GetRedirectUrl('AdminEditWholesaleDiscounts'));
		$this->RenderTemplate($template);
		return true;
	}
}